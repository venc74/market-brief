...
(open/trailing) позиция за същия тикър от последните
config.BACKTEST_DEDUP_WINDOW_DAYS дни — тогава новият сигнал се третира
като продължение на съществуващата, не нова сделка (иначе същата реална
позиция би се преброила двойно в статистиката, ако pivot/target леко се
е изместил между две последователни Action появявания).
...
# ──────────────────────────────────────────────────────────────────────────
def _has_recent_live_position(tracker: dict, ticker: str, new_entry_date: str) -> bool:
    """
    True ако вече има open/trailing запис за същия тикър, чиято entry_date е
    в последните config.BACKTEST_DEDUP_WINDOW_DAYS дни спрямо новата дата —
    третираме новия сигнал като продължение, не нова позиция.
    """
    try:
        new_dt = dt.date.fromisoformat(new_entry_date)
    except ValueError:
        return False
    for rec in tracker.values():
        if rec.get("ticker") != ticker or rec.get("status") not in _LIVE_STATUSES:
            continue
        try:
            existing_dt = dt.date.fromisoformat(rec["entry_date"])
        except (KeyError, ValueError):
            continue
        if 0 <= (new_dt - existing_dt).days <= config.BACKTEST_DEDUP_WINDOW_DAYS:
            return True
    return False
...
def _ingest_new_positions(tracker: dict) -> None:
    for path in _snapshot_files():
        ...
        for c in snap.get("action", []) or []:
            ...
            if key in tracker:
                continue
            if _has_recent_live_position(tracker, ticker, entry_date):
